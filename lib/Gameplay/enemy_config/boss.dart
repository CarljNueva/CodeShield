...
class BossHealthBar extends PositionComponent
  ...
  void render(Canvas canvas) {
    ...
    // Background (Red)
    canvas.drawRect(
      Rect.fromLTWH(0, -20, 100, 10),
      Paint()..color = Colors.red,
    );
    // Foreground (Green)
    canvas.drawRect(
      Rect.fromLTWH(0, -20, 100 * percent, 10),
      Paint()..color = Colors.green,
    );
  }
}

class BossFight extends SpriteAnimationComponent
  ...
  BossFight({required Vector2 position})
    : super(
        position: position,
  ...
  Future<void> onLoad() async {
    ...
    add(
      RectangleHitbox(
        size: size * 0.50, // Shrink the box to 70% of the 250x250 size
